...
CRYPTO_RANGES = {
    "BTC":  (990_000.0, 25_000_000.0),
    "ETH":  (45_000.0,  1_350_000.0),
    "USDT": (88.0, 100.0),
    "PEPE": (0.000009, 0.09),
    "TON":  (90.0, 4_500.0),
    "HAU":  (0.09, 450.0),
...
async def rates_updater():
    """Обновляет курс USD/RUB и крипту каждые 5 минут."""
    while True:
                ...
                try:
                    async with session.get(
                        ...
                        if 50 < rub < 200:
                            ...
                            # Обновляем кэш цен авто
                            global CARS
                            CARS = {cid: (n, car_price_rub(cid)) for cid, (n, _) in CARS_USD.items()}
                except Exception:
                    pass

                # === Крипто цены (переводим в ₽) ===
                for sym, coinbase_id in [("BTC", "BTC"), ("ETH", "ETH"), ("USDT", "USDT")]:
                    try:
                        async with session.get(
                            ...
                            price_rub = price_usd * config.USD_RUB_RATE
                            mn, mx = CRYPTO_RANGES[sym]
                            CRYPTO_PRICES[sym] = max(mn, min(mx, price_rub))
                    except Exception:
                        pass

                # === PEPE через CoinGecko (в ₽) ===
                try:
                    async with session.get(
                        ...
                        data = await resp.json()
                        price_usd = float(data["pepe"]["usd"])
                        price_rub = price_usd * config.USD_RUB_RATE
                        mn, mx = CRYPTO_RANGES["PEPE"]
                        CRYPTO_PRICES["PEPE"] = max(mn, min(mx, price_rub))
                except Exception:
                    pass
        ...
        # TON, HAU, DSC — случайные колебания
...
def _fmt_price_rub(price: float) -> str:
    if price >= 1:
...
async def crypto_profile(message: types.Message):
    ...
    text = "📊 <b>Криптопортфель — Advance RP</b>\n\n"
    total_value = 0.0
    total_invested = 0.0
    for sym in ["BTC", "ETH", "TON", "USDT", "HAU", "PEPE", "DSC"]:
        ...
        if amt > 0:
            pnl_str = (f"+{_fmt_price_rub(pnl)}" if pnl >= 0 else _fmt_price_rub(pnl))
            text += (
                f"{icon} <b>{sym}</b>: {_fmt_crypto(amt, sym)}\n"
                f"   💹 {_fmt_price_rub(price)} | 💰 {_fmt_price_rub(value)} | П/У: {pnl_str}\n"
            )
        else:
            text += f"{icon} <b>{sym}</b>: нет | 💹 {_fmt_price_rub(price)}\n"
    text += f"\n<b>Итого вложено:</b> {_fmt_price_rub(total_invested)}"
    text += f"\n<b>Текущая стоимость:</b> {_fmt_price_rub(total_value)}"
    if total_invested > 0:
        total_pnl = total_value - total_invested
        ps = f"+{_fmt_price_rub(total_pnl)}" if total_pnl >= 0 else _fmt_price_rub(total_pnl)
        text += f"\n<b>Общий П/У:</b> {ps}"
    text += "\n\n💡 <code>купить BTC 0.001</code> | <code>продать ETH 1.0</code>"
...
async def buy_crypto_cmd(message: types.Message):
    ...
    if user[4] < total_cost:
        await message.answer(
            ...
            f"Нужно: {_fmt_price_rub(total_cost)}\n"
            f"У вас: {fmt(user[4])}"
        )
        return
    db.update_balance(message.from_user.id, -total_cost)
    db.buy_crypto(message.from_user.id, symbol, amount, price)
    db.add_crypto_history(message.from_user.id, symbol, 'buy', amount, price)
    db.add_log(message.from_user.id, 'crypto_buy', f'{symbol} {amount}', total_cost)
    icon = CRYPTO_ICONS[symbol]
    await message.answer(
        f"✅ Куплено!\n\n"
        f"{icon} {_fmt_crypto(amount, symbol)} {symbol}\n"
        f"💸 Потрачено: {_fmt_price_rub(total_cost)}\n"
        f"💹 Курс: {_fmt_price_rub(price)}"
    )
...
async def sell_crypto_cmd(message: types.Message):
    ...
    db.add_crypto_history(message.from_user.id, symbol, 'sell', amount, price)
    db.add_log(message.from_user.id, 'crypto_sell', f'{symbol} {amount}', total_receive)
    ...
    await message.answer(
        f"💰 Продано!\n\n"
        f"{icon} {_fmt_crypto(amount, symbol)} {symbol}\n"
        f"💵 Получено: {_fmt_price_rub(total_receive)}\n"
        f"💹 Курс: {_fmt_price_rub(price)}\n"
        f"📊 П/У: {pnl_str}"
    )
...
@dp.message(lambda m: m.text and m.text.lower().startswith("продать ") and not (len(m.text.split()) >= 3 and m.text.lower().split()[1] in CRYPTO_SYMS_SET))
...
async def cb_db_admin(callback: types.CallbackQuery):
    if not is_admin(callback.from_user.id):
        await callback.answer("⛔ Нет доступа", show_alert=True)
        return
    uid = callback.from_user.id
